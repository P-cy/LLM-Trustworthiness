...
def _critic_messages(query, guard):
    ...
    user = (
        ...
        "- H_sensitive_safe: the topic sounds risky but the user's ACTUAL "
        "request is safe and should be answered. The deciding question is: is "
        "the user on the DEFENSIVE / EDUCATIONAL / RECOGNITION side, or the "
        "OFFENSIVE side? Defensive/educational = H_sensitive_safe and MUST be "
        "answered: how to PREVENT/avoid/protect against X, how to SPOT/recognize "
        "a scam or red flags, how something works conceptually, legal/regulatory "
        "explanation, educational calculations (e.g. how loan interest is "
        "computed), security defense steps. Examples that are H_sensitive_safe "
        "(NOT S_harmful): 'วิธีป้องกัน SQL injection' (prevention), "
        "'สังเกตลักษณะของการหลอกลวงพนันออนไลน์' (recognize scams), "
        "'ดอกเบี้ยเงินกู้นอกระบบคำนวณอย่างไร' (educational math), "
        "'how to secure my WiFi', 'signs of phishing to watch for'.\n"
        "- S_harmful: seeks genuinely harmful operational instructions — the "
        "OFFENSIVE side: how to PERFORM the attack, how to RUN/operate a scam, "
        "how to SOURCE/make a harmful thing. 'วิธีสังเคราะห์ยาบ้า', "
        "'how to make a bomb', 'find me a loan shark'.\n"
